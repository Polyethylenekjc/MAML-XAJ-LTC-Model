...
class MLExperimentPipeline:
    ...
    def analyze_results(self, all_predictions):
        """
        Analyzes prediction results for all datasets and models.
        Supports ShapLimeAnalyzer, ShapAnalyzer, and others.
        """
        for analyzer in self.analyzers:
            for dataset_name, predictions in all_predictions.items():
                ...
                for model_name, result in predictions.items():
                    ...
                    try:
                        # ---- SHAP + LIME 统一分支 ----
                        if analyzer_class_name in ['ShapLimeAnalyzer', 'ShapAnalyzer']:
                            configured_model_name = getattr(analyzer, 'model_name', None)
                            # 修改逻辑：如果是 ShapLimeAnalyzer，则不需要模型名匹配，允许对所有模型使用
                            # 如果是 ShapAnalyzer，则仍需要模型名匹配
                            if analyzer_class_name == 'ShapAnalyzer' and configured_model_name and configured_model_name != model_name:
                                self.logger.info(
                                    f"Skipping {analyzer_class_name} for {model_name} on {dataset_name}: "
                                    f"configured for model {configured_model_name}",
                                    module="MLExperimentPipeline"
                                )
                                continue
                            ...
                            if model_name in self.models:
                                ...
                                self.logger.warning(
                                    f"Skipping {analyzer_class_name} for {model_name}: model not found or not configured",
                                    module="MLExperimentPipeline"
                                )
                        ...
                        # ---- 递归预测分析 ----
                            ...
                            if model and model_name in self.models:
                                ...
                                self.logger.warning(
                                    f"Skipping recursive prediction analysis for {model_name}: model not found",
                                    module="MLExperimentPipeline"
                                    )
                        ...
                        self.logger.error(
                            f"Error analyzing {model_name} on {dataset_name} with {analyzer_class_name}: {str(e)}",
                            module="MLExperimentPipeline"
                        )
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error saving results for {model_name} on {dataset_name} with {analyzer_class_name}: {str(e)}",
                            module="MLExperimentPipeline"
                        )
