...
class ConfigManagerGUI:
    ...
    def show_model_detail(self, model_name):
        """右侧显示指定模型的参数配置"""
        # 清空旧内容
        for widget in self.model_detail_frame.winfo_children():
            widget.destroy()

        model_config = self.config_data['models'][model_name]

        ttk.Label(self.model_detail_frame, text=f"Model: {model_name}", font=("Arial", 12, "bold")).pack(anchor=tk.W, pady=(0, 10))

        # 启用状态
        enabled_var = tk.BooleanVar(value=model_config.get('enabled', False))
        ttk.Checkbutton(self.model_detail_frame, text="Enable Model", variable=enabled_var,
                        command=lambda: self.update_model_enabled(model_name, enabled_var)).pack(anchor=tk.W, pady=(0, 10))

        # 左右两列布局
        content_frame = ttk.Frame(self.model_detail_frame)
        content_frame.pack(fill=tk.BOTH, expand=True)
        content_frame.columnconfigure((0, 1), weight=1)

        # 参数配置
        params_frame = ttk.LabelFrame(content_frame, text="Model Parameters", padding="10")
        params_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N), padx=(0, 10), pady=(0, 10))
        ...
        # Trainer 配置
        trainer_frame = ttk.LabelFrame(content_frame, text="Trainer Parameters", padding="10")
        trainer_frame.grid(row=0, column=1, sticky=(tk.W, tk.E, tk.N), padx=(10, 0), pady=(0, 10))
    ...
    def show_model_config(self):
        """
        改进版：使用垂直列表栏（Listbox）显示模型，而不是 Notebook 标签页
        """
        model_frame = ttk.LabelFrame(self.module_content_frame, text="Model Configuration", padding="10")
        model_frame.pack(fill=tk.BOTH, expand=True)

        # 创建左右布局
        left_frame = ttk.Frame(model_frame, width=220)
        ...
        # 模型列表 Listbox
        ttk.Label(left_frame, text="Models", font=("Arial", 11, "bold")).pack(anchor=tk.W, pady=(0, 5))
        self.model_listbox = tk.Listbox(left_frame, height=25, exportselection=False)
        ...
        ttk.Button(btn_frame, text="Enable", command=self.enable_selected_model).pack(side=tk.LEFT, expand=True, padx=2)
        ttk.Button(btn_frame, text="Disable", command=self.disable_selected_model).pack(side=tk.LEFT, expand=True, padx=2)

        # 模型详情显示区域
        self.model_detail_frame = ttk.Frame(right_frame)
        self.model_detail_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def enable_selected_model(self):
        selection = self.model_listbox.curselection()
        if selection:
            model_name = self.model_listbox.get(selection[0])
            self.config_data['models'][model_name]['enabled'] = True
            self.show_model_detail(model_name)

    def disable_selected_model(self):
        selection = self.model_listbox.curselection()
        if selection:
            model_name = self.model_listbox.get(selection[0])
            self.config_data['models'][model_name]['enabled'] = False
            self.show_model_detail(model_name)
    ...
    def update_model_tabs(self):
        ...
        if hasattr(self, "model_listbox"):
            ...
            for model_name in self.config_data['models'].keys():
                self.model_listbox.insert(tk.END, model_name)

        # 默认显示第一个模型
        if self.config_data['models']:
            first_model = list(self.config_data['models'].keys())[0]
            self.show_model_detail(first_model)
